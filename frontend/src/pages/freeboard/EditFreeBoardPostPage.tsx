...
import FreeBoardContentInput from '@pages/community/FreeBoardContentInput';
import FreeBoardTitleInput from '@pages/community/FreeBoardTitleInput';
import {
	FreeBoardPostResponseType,
	FreeBoardWriteType,
} from '@typedef/community/freeboard.types';
import { readFileAsDataURL } from '@utils/eureka';
...
const EditFreeBoardPostPage: React.FC = () => {
	// 연결하기
	const { id } = useParams();
	...
	// post 시 보내야 하는 body
	const [postInput, setPostInput] = useState<FreeBoardWriteType>({
		title: '',
		content: '',
	});
	...
	const onClickSubmitButton = useCallback(async () => {
		...
		authHttp
			.post(`freeboard/posts/update/${id}`, formData, {
				'Content-Type': 'multipart/form-data',
			})
			.then(() => navigation('/community/free'));
...
export default EditFreeBoardPostPage;
